...
const boolean=(name,fallback=false)=>{const value=values[name];if(value===undefined)return fallback;if(!['0','1'].includes(value))throw new Error(`--${name} must be 0 or 1`);return value==='1';};
...
if(resumePath){
  ...
  network=createTwoModalityPaperNetwork({neuronsPerModality:integer('neurons',71,3),seed:integer('network-seed',1,0),initialRecurrentScale:number('initial-jitter',1e-5),excludeSelfCoupling:boolean('zero-diagonal',false)});
...
const experiment=priorExperiment||{scenario:scenarioName,reportedOutcome:scenario.reported,meanRadii:scenario.meanRadii,radiusSdFraction:number('radius-sd-fraction',.1),inputSeed:integer('input-seed',2,0),learningRate:number('learning-rate',scenario.learningRate),policy:values.policy||'fixed-best',batchSize:integer('batch-size',1),integrationStep:number('integration-step',.5,Number.EPSILON),tolerance:number('tolerance',1e-9,Number.EPSILON),maxSettleIterations:integer('max-settle',50000),zeroDiagonal:boolean('zero-diagonal',false),checkpointInputCount:integer('checkpoint-samples',32),checkpointSeed:integer('checkpoint-seed',3,0),checkpointInterval:integer('checkpoint-interval',25),fidelity:{radiusSdFraction:'0.1 is borrowed from the companion paper; target-paper coefficient unreported',integration:'project reconstruction choice',initialization:'target paper says near-zero cross-talk; scale and distribution unreported',selfCoupling:'high-dimensional diagonal policy unreported',checkpoint:'fixed deterministic input ensemble; ensemble size and interval unreported'}};
const zeroDiagonal=experiment.zeroDiagonal??true;
const legacyOnlineCheckpoint=priorExperiment!==null&&experiment.checkpointInputCount===undefined;
let checkpointInputs=null;
if(!legacyOnlineCheckpoint){const checkpointSampler=createPaperInputSampler({meanRadii:experiment.meanRadii,radiusSdFraction:experiment.radiusSdFraction,seed:experiment.checkpointSeed});checkpointInputs=Array.from({length:experiment.checkpointInputCount},()=>checkpointSampler());}
...
const training=network.train({sampler,steps:additionalSteps,batchSize:experiment.batchSize,learningRate:experiment.learningRate,policy:experiment.policy,restoreBest:values['restore-best']!=='0',checkpointInputs,checkpointInterval:experiment.checkpointInterval??1,legacyOnlineCheckpoint,gradientClip:limit('gradient-clip'),maxAbsWeight:limit('max-weight'),zeroDiagonal,settle:{integrationStep:experiment.integrationStep,tolerance:experiment.tolerance,maxIterations:experiment.maxSettleIterations,allowUnconverged:values['allow-unconverged']==='1'},onStep:record=>{latest=record;if(record.step%checkpointEvery===0){save(true,startingSteps+record.step);console.error(`step ${startingSteps+record.step}: objective=${record.objective} checkpoint=${record.checkpointObjective} settle=${record.meanSettleIterations}`);}}});

import {contentId, immutable, NUMERIC_CONVENTIONS, CONTENT_ID} from './provenance.js';
...
function makeShape({geometry, topology, attributes, provenance, depth = 0, createdAtStep = 0, tags = []}) {
  if(depth>128)throw Error('shape derivation depth exceeds operational cap 128');
  if(provenance.parents.some(id=>!CONTENT_ID.test(id)))throw Error('derived shapes require full SHA-256 parent IDs');
  const body = immutable({schema:'artistic-shape/v2',pipeline:'artistic-translation',stage:'geometry',numeric:NUMERIC_CONVENTIONS,units:'dimensionless-art-units',coordinateSystem:'authored-cartesian-right-handed',geometry, topology, attributes, provenance:{...provenance,implementation:'shape-operators/v2'}, depth, tags});
  const shape = immutable({id:contentId(body), ...body, createdAtStep});
...
export class ShapeCognitionEngine {
  constructor({seed=1,rng=null,beamWidth=3,maxWorkingSet=10,noveltyThreshold=.08}={}) { this.rng=rng||new SeededRandom(seed); this.seed=seed; this.beamWidth=beamWidth; this.maxWorkingSet=maxWorkingSet; this.noveltyThreshold=noveltyThreshold; this.library=new Map(); this.workingSet=[]; this.steps=[]; }
  add(shape,{working=true}={}) { verifyArtisticShape(shape); for(const parent of parentRefs.get(shape)||[])this.add(parent,{working:false}); const existing=this.library.get(shape.id); if(existing)verifyArtisticShape(existing); this.library.set(shape.id,shape); if(working&&!this.workingSet.some(x=>x.id===shape.id))this.workingSet.push(shape); return shape; }
  ...
  step(goal,candidateLimit=24){const candidates=this.generateCandidates(candidateLimit).map(shape=>({shape,fitness:scoreShape(shape,{library:this.library,workingSet:this.workingSet,goal})})).sort((a,b)=>b.fitness.total-a.fitness.total||a.shape.id.localeCompare(b.shape.id)); const selected=candidates.slice(0,this.beamWidth); for(const x of selected)if(x.fitness.novelty>=this.noveltyThreshold){this.add(x.shape,{working:false});const committed=Object.freeze({...x.shape,createdAtStep:this.steps.length+1});parentRefs.set(committed,parentRefs.get(x.shape)||[]);this.library.set(x.shape.id,committed);} this.workingSet=[...selected.map(x=>x.shape),...this.workingSet].filter((x,i,a)=>a.findIndex(y=>y.id===x.id)===i).slice(0,this.maxWorkingSet); const eventBody={schema:'artistic-selection-event/v1',previousEvent:this.steps.at(-1)?.id??null,pipeline:'artistic-translation',candidates:candidates.map(x=>({shape:x.shape,fitness:x.fitness})),index:this.steps.length+1,goalId:goal.id,selected:selected.map(x=>({shapeId:x.shape.id,fitness:x.fitness})),candidateCount:candidates.length,workingSet:this.workingSet.map(x=>x.id),rngState:this.rng.snapshot?this.rng.snapshot():this.rng.state}; const record=immutable({id:contentId(eventBody),...eventBody}); this.steps.push(record); return record;}
  ...
  serialize(){const snapshot={schema:'shape-cognition/v2',pipeline:'artistic-translation',identity:'sha256/v1',seed:this.seed,operators:OPERATOR_KINDS,shapes:[...this.library.values()],steps:this.steps,active:this.workingSet.map(x=>x.id),render:{coordinateSystem:'normalized-tramline',projection:'ground-plane',order:'createdAtStep'}};return immutable({manifestId:contentId(snapshot),...snapshot});}
}

/** createdAtStep is occurrence metadata; step records bind chronology separately. */
export function verifyArtisticShape(shape){const {id,createdAtStep,...body}=shape;if(shape.pipeline!=='artistic-translation'||!CONTENT_ID.test(id)||contentId(body)!==id)throw Error('invalid artistic shape content');return true;}
